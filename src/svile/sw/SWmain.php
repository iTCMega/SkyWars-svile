...
class SWmain extends PluginBase
{
            ...
            foreach (scandir($this->getServer()->getDataPath() . "\x77\x6f\x72\x6c\x64\x73") as $worldDir) {
                if (is_dir($this->getServer()->getDataPath() . "\x77\x6f\x72\x6c\x64\x73\x2f" . $worldDir) && is_file($this->getServer()->getDataPath() . "\x77\x6f\x72\x6c\x64\x73\x2f" . $worldDir . "\x2f\x6c\x65\x76\x65\x6c\x2e\x64\x61\x74")) {
                    ...
                    if (array_key_exists("\x44\x61\x74\x61", $levelData) && $levelData["\x44\x61\x74\x61"] instanceof Compound) {
                        ...
                        $this->getLogger()->critical('There is a problem with the "level.dat" of the world: Â§f' . $worldDir);
                        unset($worldDir, $levelData, $nbt);
                    }
                }
            }
        } catch (\Exception $e) {
            $this->getLogger()->critical($e->getMessage() . ' in Â§b' . $e->getFile() . 'Â§c on line Â§b' . $e->getLine());
        }
    }
    ...
    public function onEnable()
    {
        ...
        */
        $this->configs = new Config($this->getDataFolder() . 'SW_configs.yml', CONFIG::YAML, [
            ...
            'reward.value' => 100,
            'reward.command' => '/',
            '1st_line' => 'Â§lÂ§c[Â§bSWÂ§c]',
            '2nd_line' => 'Â§lÂ§e{SWNAME}',
            'sign.tick' => false,
            'sign.knockBack' => true,
        ...
        */
        $this->lang = new Config($this->getDataFolder() . 'SW_lang.yml', CONFIG::YAML, [
            'banned.command.msg' => '@bâ@cYou can\'t use this command here',
            'sign.game.full' => '@bâ@cThis game is full, please wait',
            'sign.game.running' => '@bâ@cThe game is running, please wait',
            'game.join' => '@bâ@f{PLAYER} @ejoined the game @b{COUNT}',
            'popup.countdown' => '@bThe game starts in @f{N}',
            'chat.countdown' => '@bâ@7The game starts in @b{N}',
            'game.start' => '@bâ@dThe game start now, good luck !',
            'no.pvp.countdown' => '@bYou can\'t PvP for @f{COUNT} @bseconds',
            'game.chest.refill' => '@bâ@aChests has been refilled !',
            'game.left' => '@fâ@7{PLAYER} left the game @b{COUNT}',
            'death.player' => '@câ@f{PLAYER} @cwas killed by @f{KILLER} @b{COUNT}',
            'death.arrow' => '@câ@f{PLAYER} @cwas killed by @f{KILLER} @b{COUNT}',
            'death.void' => '@câ@f{PLAYER} @cwas killed by @fVOID @b{COUNT}',
            'death.lava' => '@câ@f{PLAYER} @cwas killed by @fLAVA @b{COUNT}',//TODO: add more?
            'death.spectator' => '@fâ@bYou are now a spectator!_EOL_@fâ@bType @f/sw quit @bto exit from the game',
            'server.broadcast.winner' => '@0â@f{PLAYER} @bwon the game on SW: @f{SWNAME}',
            'winner.reward.msg' => '@fâ@bYou won @f{VALUE}$_EOL_@fâ@7Your money: @f{MONEY}$'
        ]);
        ...
        if ($this->configs['reward.winning.players']) {
            ...
            if ($this->economy->getApiVersion()) {
                $this->getLogger()->info('Â§aUsing: Â§f' . $this->economy->getApiVersion(true) . 'Â§a as economy api');
            } else {
                ...
                $this->getLogger()->critical('EconomyAPI Â§42.0.9');
                $this->getLogger()->critical('PocketMoney Â§44.0.1');
                $this->getLogger()->critical('MassiveEconomy Â§41.0 R3');
                $this->economy = null;
            }
        }
    ...
    */
     ...
     * @param string $state
     */
    public function refreshSigns($all = true, $SWname = '', $players = 0, $slot = 0, $state = 'Â§fTap to join')
    {
        if (!$all) {
            ...
            if (count($ex) == 0b100) {
                ...
                if ($this->getServer()->getLevelByName($ex[0b11]) != null) {
                    $tile = $this->getServer()->getLevelByName($ex[0b11])->getTile(new Vector3((int) $ex[0], (int) $ex[1], (int) $ex[0b10]));
            ...
            foreach ($this->signs as $key => $val) {
                ...
                if ($this->getServer()->getLevelByName($ex[0b11]) instanceof \pocketmine\level\Level) {
                    $tile = $this->getServer()->getLevelByName($ex[0b11])->getTile(new Vector3((int) $ex[0], (int) $ex[1], (int) $ex[2]));
